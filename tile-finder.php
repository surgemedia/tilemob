	...
	<div id="body" class="body">
		...
		<div id="body_right" class="body_right">
			...
			if(!empty($_GET['ke'])){
                               ...
                               else if($search_keywords=="Kitchen" || $search_keywords=="kitchen" )
				{
					$query .= 'MATCH(`Code`,`Desc`,`CategoryDescription`,`Notepad2`) AGAINST(\''.mysql_real_escape_string($search_keywords).',Wall Tile*\' IN BOOLEAN MODE) AND (Heading LIKE "WALL" OR Heading LIKE "DECORATIVE")  AND WebExport LIKE \'YES\' AND is_active=\'1\' AND';
                                ...
                                {
                                    $query .= 'MATCH(`Code`,`Desc`,`CategoryDescription`,`Notepad2`) AGAINST(\''.mysql_real_escape_string($search_keywords).'\' IN BOOLEAN MODE) AND WebExport LIKE \'YES\' AND is_active=\'1\' OR (`Use` LIKE "EXTFLOOR" OR `Use` LIKE "INT&EXT")  AND WebExport LIKE \'YES\' AND is_active=\'1\' AND';
                               ...
                               else if($search_keywords == "Pool" || $search_keywords=="Pool")
                                {
                                    ...
                                    $query .= "`Code` LIKE '%$search_keywords%'  AND WebExport LIKE 'YES' AND is_active=1 OR `Desc` LIKE '%$search_keywords%' AND WebExport LIKE 'YES' AND is_active=1  OR `CategoryDescription` LIKE '%$search_keywords%'  AND WebExport LIKE 'YES' AND is_active=1 OR `Notepad2` LIKE '%$search_keywords%'  AND WebExport LIKE 'YES' AND is_active=1 AND ";
                                    $reset_query_counter = 1;                                 
				}
			...
			if(!empty($_GET['ca'])){
				...
				$query_select .= '`Use`=\''.mysql_real_escape_string($search_category).'\' AND ';
			...
			if(!empty($_GET['su'])){
				...
				$query_select .= '`Surface`=\''.mysql_real_escape_string($search_surface).'\' AND ';
			...
			if(!empty($_GET['co'])){
				...
				$query_select .= '`Colour`=\''.mysql_real_escape_string($search_colour).'\' AND ';
			...
			if(!empty($_GET['si'])){
				...
				$query_select .= '`Size`=\''.mysql_real_escape_string($search_size).'\' AND ';
			...
			if(!empty($_GET['pe'])){
				...
				$query_select .= '`PeiRating`=\''.mysql_real_escape_string($search_peirating).'\' AND ';
			...
			if(!empty($_GET['ty'])){
				...
				$query_select .= '`Type`=\''.mysql_real_escape_string($search_type).'\' AND ';
			...
			if(!empty($_GET['pa'])){
				...
				$query_select .= '`Pattern`=\''.mysql_real_escape_string($search_pattern).'\' AND ';
			...
			if(!empty($_GET['ma'])){
				...
				$query_select .= '`Material`=\''.mysql_real_escape_string($search_material).'\' AND ';
			...
			if(!empty($_GET['th'])){
				...
				$query_select .= '`Thickness`=\''.mysql_real_escape_string($search_thickness).'\' AND ';
			...
			if(!empty($_GET['ed'])){
				...
				$query_select .= '`Edge`=\''.mysql_real_escape_string($search_edge).'\' AND ';
			...
			if(!empty($_GET['sl'])){
				...
				$query_select .= '`SlipRating`=\''.mysql_real_escape_string($search_sliprating).'\' AND ';
			...
			if(!empty($_GET['pr'])){
				...
				$query_select .= '(RetailPriceM2 <= '.$search_pricerange.') AND';
...
//				echo "SELECT * FROM shop_webitems WHERE $query WebExport='YES' AND is_active='1'".'<br/>'; exit;
				...
				$reset_query_select =  substr($query_select, 0, -4);
				if($reset_query_counter == 1){
					if($reset_query_select!=""){
						$new_query = "`Code` LIKE '%$search_keywords%'  AND WebExport LIKE 'YES' AND is_active=1 AND ".$reset_query_select." 
						OR `Desc` LIKE '%$search_keywords%' AND WebExport LIKE 'YES' AND is_active=1 AND ".$reset_query_select." 
						OR `CategoryDescription` LIKE '%$search_keywords%'  AND WebExport LIKE 'YES' AND is_active=1 AND ".$reset_query_select."
						OR `Notepad2` LIKE '%$search_keywords%'  AND WebExport LIKE 'YES' AND is_active=1 AND ".$reset_query_select."";
					}else{
						$new_query = "`Code` LIKE '%$search_keywords%'  AND WebExport LIKE 'YES' AND is_active=1 
						OR `Desc` LIKE '%$search_keywords%' AND WebExport LIKE 'YES' AND is_active=1  
						OR `CategoryDescription` LIKE '%$search_keywords%'  AND WebExport LIKE 'YES' AND is_active=1 
						OR `Notepad2` LIKE '%$search_keywords%'  AND WebExport LIKE 'YES' AND is_active=1 ";
					}
					$result_webitems = mysql_query("SELECT * FROM shop_webitems WHERE ".$new_query) or die(mysql_error());
					$page_query = $new_query;
				}else{
					$result_webitems = mysql_query("SELECT * FROM shop_webitems WHERE ".$reset_query) or die(mysql_error());
					$page_query = $reset_query;
				}
			...
			<div id="gallery" class="gallery">
				...
				if($total_results>0) {
					...
					$result_webitems = mysql_query("SELECT *, WebPricePce, TradePricePce, (WebPricePce+TradePricePce) as pricesum FROM shop_webitems WHERE ".$page_query." ORDER BY pricesum ASC LIMIT ".$start_row.",".$results_per_page."") or die(mysql_error());
...
// echo $reset_query_counter.'<br/>';
// if($reset_query_counter == 1){
// 	echo $reset_query_select.'<br/>';
//     $query = "`Code` LIKE '%$search_keywords%'  AND WebExport LIKE 'YES' AND is_active=1 AND ".$reset_query_select." 
//     OR `Desc` LIKE '%$search_keywords%' AND WebExport LIKE 'YES' AND is_active=1 AND ".$reset_query_select." 
//     OR `CategoryDescription` LIKE '%$search_keywords%'  AND WebExport LIKE 'YES' AND is_active=1 AND ".$reset_query_select."
//     OR `Notepad2` LIKE '%$search_keywords%'  AND WebExport LIKE 'YES' AND is_active=1 AND ".$reset_query_select."";
// }
