...
<div id="container" class="container">
	...
	<div id="body" class="body">
		...
		function fixback($string)
		{
			...
			$total_results = 0;
			if($_GET['material']!=""){
				$query_request = "AND Material LIKE '".fixback($_GET['material'])."'";	
			}
			if($_GET['surface']!=""){
				$query_request = "AND Surface LIKE '".fixback($_GET['surface'])."'";	
			}
			$new_queryis = "AND Heading LIKE '".$_GET['key']."' ".$query_request;
			if($_GET['subheading']!=""){
				$new_queryis = "AND SubHeading LIKE '".fixback($_GET['subheading'])."'";	
				$getsubtitlename = mysql_query("SELECT Description FROM shop_sub_heading WHERE code LIKE '".$_GET['subheading']."'");
				$getsubtitlenamerow = mysql_fetch_array($getsubtitlename);
				$showsubtitles = $getsubtitlenamerow['Description'];
			}
				$result_webitems = mysql_query("SELECT * FROM shop_webitems WHERE WebExport LIKE 'YES' AND is_active = 1 ".$new_queryis) or die(mysql_error());
				...
				if($total_pages>1) {
					...
					if($page>1){$pagenumbers.='<div class="prev"><a href="featuredProducts.php?'.$_parse_url['query'].'&p='.($page-1).'" title="Prev">Prev</a></div>';}
					for($i=1;$i<=$total_pages;$i++) {
						if($i==$page){$page_class='page_selected';}else{$page_class='page';}
						$pagenumbers .= '<div class="'.$page_class.'"><a href="featuredProducts.php?'.$_parse_url['query'].'&p='.$i.'" title="'.$i.'">'.$i.'</a></div>';						
					}
					if($page<$total_pages){$pagenumbers.='<div class="next"><a href="featuredProducts.php?'.$_parse_url['query'].'&p='.($page+1).'" title="Next">Next</a></div>';}
					$pagenumbers .= '<div class="clear"></div></div>';
				}
			
			?>
            
			<div id="blackheading" class="blackheading"><!--Search results -->› <span><?php echo trim($resSearchFor['Description']); 
			if($_GET['surface']!=""){
				for($lsk=1; $lsk<=$listsurfacenum; $lsk++){
					if($_GET['surface']==$lscode[$lsk]){$showsubtitles=' : '.$lsdecr[$lsk];}
				}
			}
			if($_GET['material']!=""){
				for($lmk=1; $lmk<=$listmaterialnum; $lmk++){
					if($_GET['material']==$lmcode[$lmk]){$showsubtitles=' : '.$lmdecr[$lmk];}
				}
			}
			echo $showsubtitles;
			...
			<div id="gallery" class="gallery">
				...
				if($total_results>0) {
					...
					$result_webitems = mysql_query("SELECT *, WebPricePce, TradePricePce, (WebPricePce+TradePricePce) as pricesum FROM shop_webitems WHERE WebExport LIKE 'YES' AND is_active = 1 ".$new_queryis." LIMIT ".$start_row.",".$results_per_page) or die(mysql_error());
