	...
	<div id="body" class="body">
		...
		<div id="body_right" class="body_right">
			...
			$total_results = 0;
				...
				$result_webitems = mysql_query("SELECT * FROM shop_webitems WHERE WebExport LIKE 'YES' AND is_active=1 ORDER BY RAND() LIMIT 0,100;") or die(mysql_error());
			...
			<div id="gallery" class="gallery">
				...
				if($total_results>0) {
					...
					$result_webitems = mysql_query("SELECT *, WebPricePce, TradePricePce, (WebPricePce+TradePricePce) as pricesum FROM shop_webitems WHERE WebExport LIKE 'YES' AND is_active = 1 ORDER BY RAND() LIMIT $start_row,$results_per_page") or die(mysql_error());					
					while($row_webitems = mysql_fetch_array($result_webitems)) {
						...
						if($item_pcsm2>0&&$item_unit=='M2'){
							...
							if(!empty($item_webpricem2)) {
							    ...
							    $item_web_discount_amount = floatval($item_retailpricem2)*0.2; 
