...
switch ($_GET['action']) {
  ...
  // print_r($_REQUEST);
          ...
          if($orderId){
          $billing_string = '';
          $result_billing_info = mysql_query("SELECT * FROM billing_info WHERE order_id LIKE '".$_shop_order_id."'");
          ...
          $cart_string = '';
				$cart_final_total = 0;
				$result_cart = mysql_query("SELECT * FROM shop_cart WHERE order_id LIKE '".$_shop_order_id."' AND qty > 0 AND is_active = 1");
				if($_shop_total_cart>0) {
					$cart_string .= '
