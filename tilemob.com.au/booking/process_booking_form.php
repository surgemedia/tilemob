...
if($_POST['submit_form'] && $_POST['random'] == trim($_POST['textfield_sum'])) {
	...
	if($_GET['reschedule'] == "true" && isset($_GET['booking_id'])) {
		...
		if($youare!="" && $yourname!="" && $yourphone!="" && $youremail!="" && $youraddress!="" && $project_type!="") {
	...
	echo 'self.location.replace("send_booking_form.php#top");';
	echo '</script>';
} else {
	echo '<script type="text/javascript" charset="utf-8">';
	echo 'self.location.replace("booking_form.php");';
	echo '</script>';
}
...
function sendBookingEmail($booking_id, $is_reschedule, $old_bookingtime) {
	...
	if($is_reschedule == true){
		...
		$message .= "<br>Depending on the scope of your project and the number of areas you are selecting tiles for, please allow up to 90 minutes to complete your consultation in our showroom. ";
	...
	$message2 .= "<br>Depending on the scope of your project and the number of areas you are selecting tiles for, please allow up to 90 minutes to complete your consultation in our showroom. ";	
