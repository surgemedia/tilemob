...
$image_code = trim($_POST['textfield_edittile']);
$image_description = trim($_POST['textarea_newdescription']);
...
function readDirectories() {
	global $directoryList, $image_files, $image_code, $image_description, $image_folder, $total_found;
...
function writeDescription($w_directory, $w_code, $w_description) {
	...
	global $image_folder, $write_filename, $selected_category, $selected_categories;
	...
	while (!feof($fh)) { // Go through each line in file
		...
		if (!empty($currentLine)) {
			...
			if (trim($line_image_code) == trim($w_code)) { //If image_code exists in file, replace it
				if(!in_array(trim($w_code), $imageCodes)) {
					$imageCodes[] = trim($w_code);
					$stringData .= trim($w_code). ' ' .trim($w_description)."\r\n";
					$selected_category = dirname($w_directory);
	...
	if ($image_code_exist == 'no') {
		$stringData .= $w_code.' '.trim($w_description)."\r\n";
		$selected_category = dirname($w_directory);
	}
	
	//echo 'selected_category: '.$selected_category.' <br/>stringData: <br/>'.nl2br($stringData).'<br/>';
...
function proceedProcess() {
	//Declare global variables
	global $image_folder, $selected_category, $total_found, $selected_categories, $image_code;
	
	if(empty($selected_category)) {
		$selected_category = $image_folder;
		$directory = '';
	} else {
		$directory = basename($selected_category);
	}	
	
	$directory .= '/page1';
	
	$pagecount = countTotalPages($directory);
	
	//echo '<br/>directory: '.$directory.', image_code: '.$image_code.'<br/>';

	if($total_found == 1) {
		header('location:regenerate_gallery.php?select_imagelocation='.$directory.'&prev=edit_image.php');
	} else if($total_found > 1) {
		$echo_string = '<p>More than one match found, please make a choice: </p>';
		$count = 1;
		foreach($selected_categories as $key => $value) {
			$echo_string .= '<div style="clear:both;margin-top:4px;">'.$count.'. <a href="regenerate_gallery.php?select_imagelocation='.str_replace('gallery/','',$value).'/page1&prev=edit_image.php">'.$value.'</a> (#'.$image_code.')</div>';
			$count++;
		}
		echo $echo_string;
	}
