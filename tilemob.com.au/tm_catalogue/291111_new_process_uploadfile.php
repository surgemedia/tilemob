...
/*function checkFolder() {
	...
	global $image_code, $image_display, $image_folder, $product_limit;
	...
	if (count($check_this_page) < $product_limit) {
		...
		if ($file_uploaded == false) { //Only upload product image if it hasn't already been uploaded before
			uploadFiles($this_page_folder, $image_display); //Upload file
			//echo '</br>Commence upload of '.$image_display.' to folder: '.$this_page_folder;
	...
	for ($i = 0; $i < $pagecount; $i++) { //Go through all page folders in this category root folder
		...
		for ($j = 0; $j < count($check_this_page); $j++) {
			...
			//$image_number = substr($image_display, 2, (strpos($image_display, '.jpg')-1)); //Get only the product's reference number to compare
			$image_number = $image_code;
			...
			if ($image_number > $check_this_page[$j]) { //If product number is higher value than an existing product in that folder
				...
				//Upload $image_display to this folder
				if ($file_uploaded == false) { //Only upload product image if it hasn't already been uploaded before
					uploadFiles($this_page_folder, $image_display); //Upload file
					//echo '</br>Commence upload of '.$image_display.' to folder: '.$this_page_folder;
					$file_uploaded = true; //Boolean checker to make sure we only upload the product image once, turned on/off by script
					...
					if (is_dir($next_page_folder)) {
						...
						@rename($this_page_folder.'t_'.$element_to_move.'.jpg', $next_page_folder.'t_'.$element_to_move.'.jpg'); //Move Thumbnail
						...
						@rename($this_page_folder.'t_'.$element_to_move.'.jpg', $next_page_folder.'t_'.$element_to_move.'.jpg'); //Move Thumbnail
						//echo '</br> Moving file: '.$this_page_folder.'t_'.$element_to_move.'.jpg';
						@rename($this_page_folder.$element_to_move.'.jpg', $next_page_folder.$element_to_move.'.jpg'); //Move Display
						//echo '</br> Moving file: '.$this_page_folder.$element_to_move.'.jpg';
					}
				}					
			}			
	...
	if ($file_uploaded == false) { //Only upload product image if it hasn't already been uploaded before
		...
		if (is_dir($next_page_folder)) { //If the next page folder already exists, then just upload the display to the next page folder		
			uploadFiles($next_page_folder, $image_display); //Upload file
			//echo '</br>Commence upload of '.$image_display.' to folder: '.$next_page_folder;
			$file_uploaded = true; //Boolean checker to make sure we only upload the product image once, turned on/off by script
			//If the next page folder DOES NOT already exists, then we must create the page folder plus move all neccessary files 
			//from previous folder (make copies) and then upload the display to the next page folder
		} else {
			mkdir($next_page_folder, 0777);
			//echo '</br>Creating a new page folder: '.$next_page_folder;
			global $server_dir_main;
			...
			chmod($next_page_folder.'photogallery_info.txt', 0777);
			//echo '</br>---------------------------------------------------------';
			//Uploading display image to the newly created folder
			uploadFiles($next_page_folder, $image_display); //Upload file
			//echo '</br>Commence upload of '.$image_display.' to folder: '.$next_page_folder;
			$file_uploaded = true; //Boolean checker to make sure we only upload the product image once, turned on/off by script
		}
...
function checkFolder() {
	//Declare global variables
	global $server_dir_main, $image_code, $image_display, $image_folder, $product_limit;
	...
	$temp_folder = dirname($image_folder).'/temp/';
	if(!is_dir($temp_folder)) {
		mkdir($temp_folder, 0777);
	} else {
		//remove all contents from directory
		//recursiveDelete($temp_folder);
		//mkdir($temp_folder, 0777);
		/*$strtotime = strtotime('now');	
		rename($temp_folder, $strtotime);
		mkdir($temp_folder, 0777);
		mkdir($temp_folder.$strtotime, 0777);
		rename($strtotime, $temp_folder.$strtotime);*/
	}
	
	if(uploadFiles($temp_folder, $image_display)) {
		$tiles_currentpath = array(); //Create an array that will hold product images of the folder we want to check
		for ($i = 1; $i <= $pagecount; $i++) {
			$this_page_folder = $image_folder_root.'page'.$i.'/'; //This is the page1 folder we want to check
			if(is_dir($this_page_folder)) {
				$tiles_currentpath = storeDirList2($this_page_folder);
			}
		}
		
		//move all tiles to temp folder
		if(!empty($tiles_currentpath)) {
			$tiles_currentpath = bubbleSort($tiles_currentpath);
			$tiles_newpath = array();
			foreach($tiles_currentpath as $key => $value) {
				$tile_display_imgsrc = str_replace('t_','',$value);
				$tile_thumb_imgsrc = $value;
				echo 'tile_thumb_imgsrc: '.$tile_thumb_imgsrc.'<br/>';
				if(is_file($tile_thumb_imgsrc)) {
					if(rename($tile_thumb_imgsrc, $temp_folder.basename($value))) {
						$tiles_newpath[] = $temp_folder.basename($value);
						if(is_file($tile_display_imgsrc)) {
							rename($tile_display_imgsrc, $temp_folder.basename($value));				
						}
					}
				}
			}
		...
		//collect tiles from temp folder
		if(empty($tiles_newpath)) {
			$tiles_newpath = array();
			$tiles_newpath = storeDirList2($temp_folder);
		}
		
		//move tiles to page folders (each folder has a total tiles limit)
		$page = 1;
		$count = $product_limit;
		if(!empty($tiles_newpath)) {
			foreach($tiles_newpath as $key => $value) {
				$tile_display_imgsrc = str_replace('t_','',$value);
				$tile_thumb_imgsrc = $value;
				if($count >= $product_limit) {					
					//new page directory
					$parent_newpath = $image_folder_root.'page'.$page.'/';
					if(is_dir($parent_newpath)) {
						mergeTextFiles($parent_newpath.'photogallery_info.txt', dirname($temp_folder).'/photogallery_info.txt'); //copy photogallery_info.txt data to new file
						recursiveDelete($parent_newpath);					
					}
					mkdir($parent_newpath, 0777);
					echo 'parent_newpath: '.$parent_newpath.'<br/>';
					$page++;
					$count = 1; //reset
				}
				
				//copy mandatory files to page directory
				if(copy('template/index.php', $parent_newpath.'index.php')){
					chmod($parent_newpath.'index.php', 0777);
				}
				if(copy('template/iframe_display_info.php', $parent_newpath.'iframe_display_info.php')){
					chmod($parent_newpath.'iframe_display_info.php', 0777);
				}
				if(copy('template/iframe_display.php', $parent_newpath.'iframe_display.php')){
					chmod($parent_newpath.'iframe_display.php', 0777);
				}
				/*if(copy('template/photogallery_info.txt', dirname($parent_newpath).'photogallery_info.txt')){
					chmod(dirname($parent_newpath).'photogallery_info.txt', 0777);
				}*/
				
				//move tiles
				echo 'tile_thumb_imgsrc: '.$tile_thumb_imgsrc.'<br/>';
				if(is_file($tile_thumb_imgsrc)) {
					$thumb_newpath = $parent_newpath.basename($tile_thumb_imgsrc);
					echo $count.'. thumb_newpath: '.$thumb_newpath.'<br/>';
					rename($tile_thumb_imgsrc, $thumb_newpath);
					
					if(is_file($tile_display_imgsrc)) {
						$display_newpath = $parent_newpath.basename($tile_display_imgsrc);
						echo $count.'. display_newpath: '.$display_newpath.'<br/>';
						rename($tile_thumb_imgsrc, $thumb_newpath);
					}
					$count++;
				}
...
function storeDirList2($store_this_dir) {
	$dirList = array(); //Create an array to store these product images
	$handler = @opendir($store_this_dir);
    // keep going until all files in directory have been read
    while ($file = @readdir($handler)) {
        if ($file != '.' && $file != '..' && substr($file, 0, 2) == "t_") {
			...
			//echo '</br>file: '.$store_this_dir.$file_number;
		}
    }
    // tidy up: close the handler
    @closedir($handler);
...
function uploadFiles($image_folder, $image_display) {
	...
	if ($_FILES["filefield_thumbnail"]["error"] > 0) {
		...
		move_uploaded_file($_FILES["filefield_thumbnail"]["tmp_name"],$image_folder . $image_thumbnail);
...
function mergeTextFiles($file, $mergefile) {
	if(is_file($file)) {
		$getfile = file_get_contents($file);
		if(is_file($mergefile)) {
			$stringData = file_get_contents($mergefile);
		} else { $stringData = ''; }
		
		$stringData .= $getfile;
		
		$fh = fopen($mergefile, 'w') or die("can't open file");	
		fwrite($fh, $stringData);
		fclose($fh);
	}
}
