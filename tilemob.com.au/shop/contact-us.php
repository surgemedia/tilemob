...
if(!empty($_POST['fullname']) && !empty($_POST['email']) && !empty($_POST['enquiry'])) {
	...
	if(trim($_POST['security'])!=$_SESSION['image_random_value_raw']) {
		header('location:contact.php?s3=Your form could not been sent because the verification code at the end of the form was entered incorrectly. Please try again.');
	} else {
		...
		header('location:contact.php?s2=Thank you, your enquiry was sent!');
	}
}
