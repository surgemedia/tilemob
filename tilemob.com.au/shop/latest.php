	...
	<div id="body" class="body">
		...
		<div id="body_right" class="body_right">
			...
			$total_results = 0;
				...
				$result_webitems = mysql_query("SELECT * FROM shop_webitems WHERE WebExport='YES' AND is_active='1' ORDER BY RAND(), lastupdate desc LIMIT 0,100;") or die(mysql_error());
				...
				if($total_pages>1) {
					...
					if($page>1){$pagenumbers.='<div class="prev"><a href="latest.php?'.$_parse_url['query'].'&p='.($page-1).'" title="Prev">Prev</a></div>';}
					for($i=1;$i<=$total_pages;$i++) {
						if($i==$page){$page_class='page_selected';}else{$page_class='page';}
						$pagenumbers .= '<div class="'.$page_class.'"><a href="latest.php?'.$_parse_url['query'].'&p='.$i.'" title="'.$i.'">'.$i.'</a></div>';						
					}
					if($page<$total_pages){$pagenumbers.='<div class="next"><a href="latest.php?'.$_parse_url['query'].'&p='.($page+1).'" title="Next">Next</a></div>';}
			...
			<div id="gallery" class="gallery">
				...
				if($total_results>0) {
					...
					$result_webitems = mysql_query("SELECT *, WebPricePce, TradePricePce, (WebPricePce+TradePricePce) as pricesum FROM shop_webitems WHERE WebExport='YES' AND is_active='1' ORDER BY RAND(), lastupdate desc LIMIT $start_row,$results_per_page") or die(mysql_error());
