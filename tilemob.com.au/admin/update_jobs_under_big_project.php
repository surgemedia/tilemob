   ...
   <?
		$time = time();
   		if($_POST['name']!="" && $_FILES['pdf_file']['name']!="" && $_FILES['img_file']['name']!=""){
			...
			$allowedTypes2 = array(IMAGETYPE_PNG, IMAGETYPE_JPEG, IMAGETYPE_GIF);
			$detectedType = exif_imagetype($_FILES['img_file']['tmp_name']);
			if($_FILES['pdf_file']['type'] == 'application/pdf') {  
				...
				$pdfs_upload = "INSERT INTO file_projects VALUES(NULL, '".$_POST['name']."', '".$uppic1."', '".$uppic2."', '".$_POST['collection']."', 1)";
				echo $pdfs_upload;
				...
				echo "Your uploaded PDF file type is not correct!";	
				exit;
			}
	
		}else{
			$error_message = "<span>You have not upload all request file.</span>";
		}
		echo $error_message;
   ?>
      <h1>Upload new Jobs under Project Page</h1>
      ...
      if(this.img_file.value==''){ alert('Have not upload image!'); this.img_file.focus(); return false}">
         <table width="460" border="0" cellspacing="0" cellpadding="3" class="table1" align="center">
			<tr>
               <td width="120" class="table1_body"><h3>Select Project Page: </h3></td>
               ...
               <?
					...
					for($i=0; $i<$check_collect_num; $i++){
						...
						echo '<option value="'.$check_collect_row['id'].'">'.$check_collect_row['name'].'</option>';
					}
			   ?>
               </select>
               </td>
			...
			<tr>
               <td width="120" class="table1_body"><h3>Project PDF File: </h3></td>
               <td class="table1_body"><input type="file" name="pdf_file" ></td>
            </tr>
            <tr>
               <td class="table1_body"><h3>Project Image: *<br/> Just support JPG, PNG and GIF</h3></td>
