   ...
   <?
		$time = time();
   		if($_POST['name']!="" || $_FILES['pdf_file']['name']!="" || $_FILES['img_file']['name']!=""){
			...
			$pdfs_upload = "UPDATE file_pdfs SET name = '".$_POST['name']."', img_file = '".$uppic2."', pdf_file = '".$uppic1."', child_index = '".$_POST['collection']."' WHERE id = ".$_POST['hid_id'];
			$pdfs_upload_result = mysql_query($pdfs_upload);
		}
		$get_data_pdf = mysql_query("SELECT * FROM file_pdfs WHERE id = ".$_POST['client_id']." AND enable_option = 1");
		$get_data_rows = mysql_fetch_array($get_data_pdf);
   ?>
      <h1>PDFs management /Edit File</h1>
      <form id="submitform" name="submitform" method="post" action="#" enctype="multipart/form-data" onsubmit="if(this.name.value==''){ alert('Name cannot empty!'); this.name.focus(); return false} ">
         <table width="460" border="0" cellspacing="0" cellpadding="3" class="table1" align="center">
			...
			?>
			<tr>
               <td width="120" class="table1_body"><h3>Product Collection: </h3></td>
               <td class="table1_body">
               <select name="collection">
               <option value="" s>Not under any Collection page</option>
            <?
				}
			?>
               <?
				if($get_data_rows['pdf_file']!="COLLECTION"){
					$check_collect = mysql_query("SELECT * FROM file_pdfs WHERE pdf_file LIKE 'COLLECTION' ORDER BY name");
			...
			?>
         	<tr>
            	<td colspan="2">
                	<a href="<?=$get_data_rows['pdf_file'];?>"  target="_blank">Click to open the PDF</a>
                </td>
            </tr>
			<tr>
               <td width="120" class="table1_body"><h3>Upload new PDF File: </h3></td>
            ...
            <tr>
               <td class="table1_body"><h3>Upload new Display Image: *<br/> Just support JPG, PNG and GIF</h3></td>
               <td class="table1_body"><input type="file" name="img_file" ><input type="hidden" name="hid_img_file" value="<?=$get_data_rows['img_file'];?>" ></td>
            </tr>
			<tr>
               <td class="table1_body" valign="top"><h3>Display Name: *</h3></td>
