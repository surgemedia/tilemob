...
<?php
	...
	$booking_code = $num_rows+1; //number of bookings in the table + 1
	//Now we add zeroes to booking_id so it makes a 6 digit code
	...
	mysql_query("INSERT into consultation
		...
		booker_address, booker_projecttype, booker_projectaddress, booker_projectcommence, booker_referredby,
		booker_referralname, booker_preferredconsultant, booker_comments, booking_month, booking_day, booking_year, 
		...
		'$youraddress','$project_type','$project_address','$project_commence','$project_referer','$project_referer_name','$preferred_consultant',
...
function sendBookingEmail($booking_id, $is_reschedule, $old_bookingtime) {
	...
	if($is_reschedule == true){
		...
		$subject2 = "TileMob.com.au - Your Consultation Booking #".$booking_id." has been rescheduled by ".$yourname;
	} else {
	...
	//----------------------------------------------------------------------------------------------
	$booking_change_url = 'http://tilemob.com.au/booking_form.php?id='.$booking_code.'&email='.$youremail;
	...
	$message = 
		...
		<b>Note: </b>This BCC email was generated for sales@tilemob.com.au only. <br/>";
	if($is_reschedule == true){
		$message .= 
		...
		"<br><b>Preferred consultant:</b> ".$preferred_consultant.
		"<br>".
		...
		"<br>".
		"<br> We have street parking available as well as a customer car park at the rear of our building. We look forward
