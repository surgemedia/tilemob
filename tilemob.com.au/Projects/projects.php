...
<body style="width:910px;">
	<?
		if($_GET['childpage']==""){
			$pdfs = "SELECT * FROM file_projects WHERE child_index = 0 AND enable_option = 1 ORDER BY id DESC";	
			$back_to_main = '
			';
		}else{
			$pdfs = "SELECT * FROM file_projects WHERE child_index = ".$_GET['childpage']." AND enable_option = 1 ORDER BY id DESC";
			$back_to_main = '
				<div class="clearfixnow">
				<a href="projects.php" title="Back to top" style="color:red !important;">Back to main page</a>
				</div>
			';
		}
		...
		for ($i = 0; $i < $pdfs_num; $i++) {
			...
			if($pdfs_row['pdf_file']=="COLLECTION"){
				$pdf_linking = "projects.php?childpage=".$pdfs_row['id'];
				$link_open = '';
			}else{
