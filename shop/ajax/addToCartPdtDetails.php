	...
	if(intval($item_quantity)>=1) {
		...
		if($row_item = mysql_fetch_array($result_item)) { //item available
			...
			if($row_cart_sum = mysql_fetch_array($result_cart_sum)) {
                ...
                // echo "$result_cart - "."SELECT * FROM shop_cart WHERE user_id='$user_id' AND item_code='$item_code' AND is_active='1'"." - "."$enterLoop - ".$enterLoop." - "."$new_qty = $current_qty+$item_quantity - ".$new_qty."=".$current_qty."+".$item_quantity." - $sql - ".$sql;                                              
			}
		}
	}
