...
class browser extends uploader {
    ...
    protected function moveUploadFile($file, $dir) {
        ...
        if (!@move_uploaded_file($file['tmp_name'], $target) &&
            ...
            @unlink($file['tmp_name']);
            return "{$file['name']}: " . $this->label("[browser.php] Cannot move uploaded file to target folder.");*/
		
		//Added by Richard on 16/08/12 to bypass safe mode (other changes also made in other files)
		$target = $dir.'/'.$file['name'];
		if(!move_uploaded_file($file['tmp_name'], $target)) {
			@unlink($file['tmp_name']);
			return $file['name'].': '.$this->label('[browser.php] Cannot move uploaded file to target folder. dir: '.$dir.', filename: '.$filename);
