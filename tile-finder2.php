	...
	<div id="body" class="body">
		...
		<div id="body_right" class="body_right">
			...
			if(!empty($_GET['ke'])){
				...
				{
					$query .= 'MATCH(`Code`,`Desc`,`CategoryDescription`,`SupplierName`,`Notepad2`) AGAINST(\''.mysql_real_escape_string($search_keywords).',Wall Tile*\' IN BOOLEAN MODE) AND (Heading LIKE "WALL" OR Heading LIKE "DECORATIVE") AND';
                               ...
                               else if($search_keywords=="Kitchen" || $search_keywords=="kitchen" )
				{
					$query .= 'MATCH(`Code`,`Desc`,`CategoryDescription`,`SupplierName`,`Notepad2`) AGAINST(\''.mysql_real_escape_string($search_keywords).',Wall Tile*\' IN BOOLEAN MODE) AND (Heading LIKE "WALL" OR Heading LIKE "DECORATIVE") AND';
                                ...
                                {
                                    $query .= 'MATCH(`Code`,`Desc`,`CategoryDescription`,`SupplierName`,`Notepad2`) AGAINST(\''.mysql_real_escape_string($search_keywords).'\' IN BOOLEAN MODE) AND (`Use` LIKE "EXTFLOOR" OR `Use` LIKE "INT&EXT") AND';
					
                                }
                               else if($search_keywords == "Internal" || $search_keywords=="internal")
                                {
                                    $query .= 'MATCH(`Code`,`Desc`,`CategoryDescription`,`SupplierName`,`Notepad2`) AGAINST(\''.mysql_real_escape_string($search_keywords).',Wall Tile*\' IN BOOLEAN MODE) AND (`Use` LIKE "FLOOR&WALL" OR `Use` LIKE "INT&EXT") AND';
					
                                }
                               else if($search_keywords == "Pool" || $search_keywords=="Pool")
                                {
                                    $query .= 'MATCH(`Code`,`Desc`,`CategoryDescription`,`SupplierName`,`Notepad2`,`Heading`,`Use`) AGAINST(\''.mysql_real_escape_string($search_keywords).'\' IN BOOLEAN MODE) AND (`Heading` LIKE "POOL" OR `Use` LIKE "POOL") AND';
			...
			if(!empty($_GET['ca'])){
				...
				$query2 .= '`Use`=\''.mysql_real_escape_string($search_category).'\' AND ';
				$query .= '`Use`=\''.mysql_real_escape_string($search_category).'\' AND ';
				$query_use = mysql_query("SELECT * FROM shop_use WHERE Code LIKE '".$search_category."' AND is_active= 1 ");
			...
			if(!empty($_GET['su'])){
				...
				$query2 .= '`Surface`=\''.mysql_real_escape_string($search_surface).'\' AND ';
				$query .= '`Surface`=\''.mysql_real_escape_string($search_surface).'\' AND ';
				$query_surface = mysql_query("SELECT * FROM shop_surface WHERE Code LIKE '".$search_surface."' AND is_active= 1 ");
			...
			if(!empty($_GET['co'])){
				...
				$query2 .= '`Colour`=\''.mysql_real_escape_string($search_colour).'\' AND ';
				$query .= '`Colour`=\''.mysql_real_escape_string($search_colour).'\' AND ';
				$query_colour = mysql_query("SELECT * FROM shop_colour WHERE Code LIKE '".$search_colour."' AND is_active = 1");
			...
			if(!empty($_GET['si'])){
				...
				$query2 .= '`Size`=\''.mysql_real_escape_string($search_size).'\' AND ';
				$query .= '`Size`=\''.mysql_real_escape_string($search_size).'\' AND ';
                                $query_size = mysql_query("SELECT * FROM shop_size WHERE Code LIKE '".$search_size."' AND is_active= 1 ");
			...
			if(!empty($_GET['pe'])){
				...
				$query2 .= '`PeiRating`=\''.mysql_real_escape_string($search_peirating).'\' AND ';
				$query .= '`PeiRating`=\''.mysql_real_escape_string($search_peirating).'\' AND ';
				$query_peirating = mysql_query("SELECT * FROM shop_peirating WHERE Code LIKE '".$search_peirating."' AND is_active = 1 ");
			...
			if(!empty($_GET['ty'])){
				...
				$query2 .= '`Type`=\''.mysql_real_escape_string($search_type).'\' AND ';
				$query .= '`Type`=\''.mysql_real_escape_string($search_type).'\' AND ';
				$query_type = mysql_query("SELECT * FROM shop_type WHERE Code LIKE '".$search_type."' AND is_active= 1 ");
			...
			if(!empty($_GET['pa'])){
				...
				$query2 .= '`Pattern`=\''.mysql_real_escape_string($search_pattern).'\' AND ';
				$query .= '`Pattern`=\''.mysql_real_escape_string($search_pattern).'\' AND ';
				$query_pattern = mysql_query("SELECT * FROM shop_pattern WHERE Code LIKE '".$search_pattern."' AND is_active= 1 ");
			...
			if(!empty($_GET['ma'])){
				...
				$query2 .= '`Material`=\''.mysql_real_escape_string($search_material).'\' AND ';
				$query .= '`Material`=\''.mysql_real_escape_string($search_material).'\' AND ';
				$query_material = mysql_query("SELECT * FROM shop_material WHERE Code LIKE '".$search_material."' AND is_active= 1 ");
			...
			if(!empty($_GET['th'])){
				...
				$query2 .= '`Thickness`=\''.mysql_real_escape_string($search_thickness).'\' AND ';
				$query .= '`Thickness`=\''.mysql_real_escape_string($search_thickness).'\' AND ';
				$query_thickness = mysql_query("SELECT * FROM shop_thickness WHERE Code LIKE '".$search_thickness."' AND is_active= 1");
			...
			if(!empty($_GET['ed'])){
				...
				$query2 .= '`Edge`=\''.mysql_real_escape_string($search_edge).'\' AND ';
				$query .= '`Edge`=\''.mysql_real_escape_string($search_edge).'\' AND ';
				$query_edge = mysql_query("SELECT * FROM shop_edge WHERE Code LIKE '".$search_edge."' AND is_active=1");
			...
			if(!empty($_GET['sl'])){
				...
				$query2 .= '`SlipRating`=\''.mysql_real_escape_string($search_sliprating).'\' AND ';
				$query .= '`SlipRating`=\''.mysql_real_escape_string($search_sliprating).'\' AND ';
				$query_sliprating = mysql_query("SELECT * FROM shop_sliprating WHERE Code LIKE '".$search_sliprating."' AND is_active=1");
			...
			if(!empty($_GET['pr'])){
				...
				//$query .= '`SlipRating`=\''.mysql_real_escape_string($search_sliprating).'\' AND ';
				$query2 .= '(RetailPriceM2 <= '.$search_pricerange.') AND';
			...
			if($query_OR == 1){
				if($query2!=""){
					$rest = substr($query2, 0, -4);
					$add_on_query = "AND ".$rest;
				}
				
				$query_summery = "`Code` LIKE '%".$search_keywords."%'  AND WebExport LIKE 'YES' AND is_active = 1 ".$add_on_query." OR 
				`Desc` LIKE '%".$search_keywords."%'  AND WebExport LIKE 'YES' AND is_active = 1 ".$add_on_query." OR 
				`CategoryDescription` LIKE '%".$search_keywords."%' AND WebExport LIKE 'YES' AND is_active = 1 ".$add_on_query." OR 
				`SupplierName` LIKE '%".$search_keywords."%'  AND WebExport LIKE 'YES' AND is_active = 1 ".$add_on_query." OR 
				`Notepad2` LIKE '%".$search_keywords."%'  AND WebExport LIKE 'YES' AND is_active = 1 ".$add_on_query."  ";
			}else{
				$query_summery = $query;
			}
...
//				echo "SELECT * FROM shop_webitems WHERE $query WebExport='YES' AND is_active='1'".'<br/>'; exit;
				echo "SELECT * FROM shop_webitems WHERE ".$query_summery." WebExport LIKE 'YES' AND is_active = 1";
				echo "<br/>test".$query_OR.$rest;
				$result_webitems = mysql_query("SELECT * FROM shop_webitems WHERE ".$query_summery." AND WebExport LIKE 'YES' AND is_active = 1") or die(mysql_error());
